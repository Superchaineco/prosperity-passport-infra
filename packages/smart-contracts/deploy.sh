...
# Deploy badges proxy
FULL_BADGES_OUTPUT=$(forge script "$BADGES_SCRIPT_PATH" --force --rpc-url sepolia --chain sepolia --sender "$SENDER" --private-key "$PRIVATE_KEY" --ffi --json)
...
FULL_RESOLVER_OUTPUT=$(NETWORK=sepolia BADGES_ADDRESS="$BADGES_ADDRESS" forge script "$POSTBADGES_SCRIPT_PATH" --rpc-url sepolia --chain sepolia --sender "$SENDER" --private-key "$PRIVATE_KEY" --broadcast --verify --json)
...
# Deploy module
FULL_MODULE_OUTPUT=$(RESOLVER_ADDRESS="$RESOLVER_ADDRESS" forge script  "$MODULE_SCRIPT_PATH" --force  --rpc-url sepolia --chain sepolia --sender "$SENDER" --private-key "$PRIVATE_KEY" --ffi --json)
...
# Deploy post module
FULL_POSTMODULE_OUTPUT=$(RESOLVER_ADDRESS="$RESOLVER_ADDRESS" MODULE_ADDRESS="$MODULE_ADDRESS" BADGES_ADDRESS="$BADGES_ADDRESS" forge script "$POSTMODULE_SCRIPT_PATH" --rpc-url sepolia --chain sepolia --sender "$SENDER" --private-key "$PRIVATE_KEY" --broadcast --verify --json)

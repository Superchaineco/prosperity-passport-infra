 ...
 */
const parseEtherscanOracleResult = (
...
const isGasPriceFixed = (
  gasPriceConfig: GasPrice[number]
): gasPriceConfig is GasPriceFixed => {
  return gasPriceConfig.type.toUpperCase() == GAS_PRICE_TYPE.FIXED;
};

const isGasPriceFixed1559 = (
  gasPriceConfig: GasPrice[number]
): gasPriceConfig is GasPriceFixedEIP1559 => {
  return gasPriceConfig.type.toUpperCase() == GAS_PRICE_TYPE.FIXED_1559;
};

const isGasPriceOracle = (
  gasPriceConfig: GasPrice[number]
): gasPriceConfig is GasPriceOracle => {
...
const getGasParameters = (
  ...
  if (!estimation) {
    return {
      ...
      maxPriorityFeePerGas: isEIP1559
  ...
  if (
    isEIP1559 &&
    'maxFeePerGas' in estimation &&
    'maxPriorityFeePerGas' in estimation
  ) {
    return estimation;
  }
...
export const getTotalFeeFormatted = (
  ...
  return gasLimit && maxFeePerGas
    ? formatVisualAmount(
        getTotalFee(maxFeePerGas, gasLimit),
        chain?.nativeCurrency.decimals
      )
    : '> 0.001';
};

const SPEED_UP_MAX_PRIO_FACTOR = BigInt(2);

const SPEED_UP_GAS_PRICE_FACTOR = BigInt(150);
  ...
  const [gasPrice, gasPriceError, gasPriceLoading] = useAsync(
    async () => {
      ...
      if (
        ...
        gasParameters.maxPriorityFeePerGas
      ) {
        return {
          maxFeePerGas:
            gasParameters.maxFeePerGas +
            (gasParameters.maxPriorityFeePerGas * SPEED_UP_MAX_PRIO_FACTOR -
              gasParameters.maxPriorityFeePerGas),
      ...
      return {
        maxFeePerGas: gasParameters.maxFeePerGas
          ? (gasParameters.maxFeePerGas * SPEED_UP_GAS_PRICE_FACTOR) /
            BigInt(100)
          : undefined,
        maxPriorityFeePerGas: undefined,
      };

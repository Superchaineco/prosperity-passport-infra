...
export const useInitSafeCoreSDK = () => {
  ...
  const safeQuery = searchParams.getAll('safe');
  const prefixedAddress = safeQuery.length > 0 ? safeQuery[0] : null;
  const { address } = parsePrefixedAddress(prefixedAddress || '');
  const undeployedSafe = useAppSelector((state) =>
    selectUndeployedSafe(state, safe.chainId, address)
  );
  ...
  useEffect(() => {
    if (
      !safeLoaded ||
      !web3ReadOnly ||
      !sameAddress(address, safe.address.value)
    ) {

...
import { ConnectedWallet } from '../useWallet';
...
export const getConnectedWallet = (
  ...
  try {
    ...
    return {
      ...
      provider: primaryWallet.provider as any,
...
const getWalletConnectLabel = async (
...
const trackWalletType = (wallet: ConnectedWallet) => {
  getWalletConnectLabel(wallet).catch(() => null);
};
...
export const switchWallet = async (onboard: OnboardAPI) => {
  ...
  const newWalletLabel = newWallets
    ? getConnectedWallet(newWallets)?.label
    : undefined;
    ...
    if (isUnlocked === true || isUnlocked === undefined) {
      connectWallet(onboard, {
        autoSelect: {
          ...
          disableModals: isUnlocked || false,
        },
      });
    }
  }

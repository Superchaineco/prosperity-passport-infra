...
export const useSafeCreation = (
  ...
  const handleCreateSafe = useCallback(async () => {
    ...
    try {
      const tx = await getSafeCreationTxInfo(
        provider,
        owners,
        threshold,
        saltNonce,
      ...
      const gasLimit = await estimateSafeCreationGas(
        chain,
        provider,
        tx.from,
        safeParams
  ...
  const watchSafeTx = useCallback(async () => {
    if (!pendingSafe?.tx || !pendingSafe?.txHash || !web3ReadOnly || isWatching)
      return;

    setStatus(SafeCreationStatus.PROCESSING);
    setIsWatching(true);

    const txStatus = await checkSafeCreationTx(
      web3ReadOnly,
      pendingSafe.tx,
      pendingSafe.txHash,
      dispatch

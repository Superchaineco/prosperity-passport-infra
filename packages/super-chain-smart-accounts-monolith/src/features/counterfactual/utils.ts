...
} from '@/services/tx/tx-sender/sdk';
...
export const getUndeployedSafeInfo = (
  ...
  return {
    ...
    chainId,
    owners: undeployedSafe.safeAccountConfig.owners.map((owner) => ({
      value: owner,
    })),
    nonce: 0,
    ...
    fallbackHandler: {
      value: undeployedSafe.safeAccountConfig.fallbackHandler!,
    },
    version:
      undeployedSafe.safeDeploymentConfig?.safeVersion || LATEST_SAFE_VERSION,
    deployed: false,
  };
};
...
export const dispatchTxExecutionAndDeploySafe = async (
  ...
  try {
    const signedTx = await tryOffChainTxSigning(
      safeTx,
      await sdkUnchecked.getContractVersion(),
      sdkUnchecked
    );
    ...
    const deploymentTx =
      await sdkUnchecked.wrapSafeTransactionIntoDeploymentBatch(
        signedTx,
        txOptions
      );
...
export const { getStore: getNativeBalance, setStore: setNativeBalance } =
  new ExternalStore<bigint>(BigInt(0));
...
export const getCounterfactualBalance = async (
  ...
  if (provider) {
    ...
    const useCache =
      cachedBalance !== undefined && cachedBalance > BigInt(0) && !ignoreCache;
    balance = useCache
      ? cachedBalance
      : (await getWeb3ReadOnly()?.getBalance(safeAddress)) ?? BigInt(0);
...
export const createCounterfactualSafe = (
  ...
  return router.push({
    ...
    query: {
      ...
      [CREATION_MODAL_QUERY_PARM]: true,
    },
  });
};
 ...
 * @param maxAttempts
 */
async function retryGetTransaction(
  provider: Provider,
  txHash: string,
  maxAttempts = 6
) {
...
export const checkSafeActivation = async (
  ...
  try {
    ...
    const replaceableTx = startBlock
      ? txResponse.replaceableTransaction(startBlock)
      : txResponse;
...
export const checkSafeActionViaRelay = (
  taskId: string,
  safeAddress: string
) => {
  const TIMEOUT_TIME = 2 * 60 * 1000; // 2 minutes
